...
int do_fork()
{
	...
	copy_image(running->uss, segment, (32 * 1024));
	//copy_image(running->uss, segment, 0x1000);
	...
	{
		offset = 32*1024 - (2 * i);//offset = 0x1000 - (2 * i); // offset RELATIVE to current segment: end of segment - number*2 because stored as 2bytes size
 ...
 **************************************************************************/
	...
	for (i = 0; i < NFD; i++)
	{
		...
		if (child->fd[i] != 0)
		{
//printf("kfork(): child->fd[i] is set.\n");
			child->fd[i]->refCount++;
			if (child->fd[i]->mode == READ_PIPE)
			{
//printf("kfork(): BEFORE: child->fd[i]->pipe_ptr->nreader = %d\n", child->fd[i]->pipe_ptr->nreader);///////////////////////////
				child->fd[i]->pipe_ptr->nreader++;
//printf("kfork():  AFTER: child->fd[i]->pipe_ptr->nreader = %d\n", child->fd[i]->pipe_ptr->nreader);///////////////////////////
			}
			if (child->fd[i]->mode == WRITE_PIPE)
			{
//printf("kfork(): BEFORE: child->fd[i]->pipe_ptr->nwriter = %d\n", child->fd[i]->pipe_ptr->nwriter);///////////////////////////
				child->fd[i]->pipe_ptr->nwriter++;
//printf("kfork():  AFTER: child->fd[i]->pipe_ptr->nwriter = %d\n", child->fd[i]->pipe_ptr->nwriter);///////////////////////////
			}
		}
		else
		{
//printf("kfork(): child->fd[i] is NULL.\n");
		}
	}
